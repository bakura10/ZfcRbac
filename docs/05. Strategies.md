...
This strategy allows to redirect any unauthorized request to another route, by optionally saving the previous
URL in a session container.
...
return [
    'zfc_rbac' => [
        'redirect_strategy' => [
            ...
            'previous_uri_session_key'       => 'redirectTo'
        ],
    ]
];
```
...
of course) with the previous URL saved into the session : http://www.example.com/login

The session container has a `zfc_rbac` name, and the URI is persisted with the key defiend in `previous_uri_session_key`
option.

> In ZfcRbac 2.4 and lower, it used to append the URI key in a query param, but for security reasons, we now save it
in session to avoid an attacker to exploit the redirect key.
